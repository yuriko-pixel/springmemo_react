...
const Memo = (props)=> {
  ...
  if(props.props === undefined) {
   ...
   if(props.props !== undefined) {console.log(props.props[0].length);}
  });
if(props.props[0] !== undefined ){
    for (let index = 0; index < props.props[0].length; index++) {
        notes.push(
          <div className="memo">
            <Link to={'/Memo/'+props.props[index].memoid} key={Math.random()}>
                <h1>{props.props[index].memotitle}</h1>
                <Moment format="YYYY/MM/DD">{props.props[index].memodate}</Moment>
                <div
                    dangerouslySetInnerHTML={{ __html: props.props[index].memohtml }}
                    className="text"
                    key={Math.random()}
                ></div>
            </Link>
